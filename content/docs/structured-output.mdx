...
- A run that ends before `done` but leaves a schema-valid store is still recorded as delivered, with a feed entry saying so. Null or partial values may be valid; usefulness remains the caller's judgement.
