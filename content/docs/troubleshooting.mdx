...
- **`Stopped: Cost $X exceeded budget $Y`:** the `maxCostUsd` cap fired. The answer store's content up to that point is preserved in `output`. Raise the cap, or reduce spend; see [cost control](/docs/cost).
