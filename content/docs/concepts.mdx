...
- `read_pages` opens up to 48 URLs in parallel tab waves, waits for each page (and, when asked, its embedded panel) to genuinely render, and returns `{url, title, text, jsonld, links}` per page, retrying failures and detecting shell reads. It also prefills a draft row per page against your output schema. Covered in depth in [structured output](/docs/structured-output).
