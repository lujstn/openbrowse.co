...
Port allocation is deterministic per slot: displays start at `:10`, VNC ports at `5900` plus the display number, noVNC ports at `6080`, and CDP ports at `9222`. The number of slots is `MAX_CONCURRENT_SESSIONS` (default 1). Over the cap the create call still returns straight away: the session is accepted and its run waits for a slot inside its own task, so a burst of submissions queues rather than blocking the caller or failing. A keep-alive session parked between follow-ups gives its slot up to a newly started run, oldest first, so a browser nobody is using cannot hold a one-slot host indefinitely. Concurrent runs may share one browser profile: each uses a private storage-state copy and merges only its own changes back when it closes.

If a `profileId` was given, the profile's storage state (cookies plus per-origin `localStorage` and `sessionStorage`) is copied into the browser before the run. When the browser closes, a three-way merge applies that session's changes to the profile without discarding unrelated changes from other sessions. See [profiles](/docs/profiles).
...
One deliberate asymmetry: a run that ends before calling `done` but leaves behind a schema-valid answer store is recorded as delivered, with a feed entry saying so. `isTaskSuccessful` reports delivery rather than judging whether null or partial values make the result useful.
...
- **`failureKind` can be null on a failure.** A session interrupted by a server restart is reconciled outside the classified path and carries no kind. `lastStepSummary` and the feed are the source of truth there.
- **`failureKind` can be set when a result was delivered.** A run stopped by its cost cap that salvaged a schema-valid answer store has `isTaskSuccessful: true` *and* still carries `budget_exceeded`, because it delivered a result without finishing on its own terms. Read the two fields together.

`isTaskSuccessful` answers a narrower question than its name may suggest: did this run deliver the requested shape? A run that reaches the end succeeds when it has no output schema, or when its output matches the requested schema. A paywall, a field the site does not publish and an agent expressing doubt can therefore accompany `true`; null and partial values in `output`, plus the agent's note in `lastStepSummary`, tell the caller what was unobtainable. `false` means nothing was delivered because the run stopped early, exhausted its steps or returned the wrong shape. Whether a delivered result is good enough remains the caller's decision.
