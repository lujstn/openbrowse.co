---
title: "Installation"
description: "Full installation of OpenBrowse on Debian or Ubuntu, including the virtual display behind the live view, configuration, and running it under systemd."
---

The quick start on the [getting started](/docs) page is enough to try OpenBrowse. This page covers the full install: the system packages behind the live browser view, every configuration variable the application actually reads, and running it as a service that survives reboots.
...
The two `tailscale` lines bring the public tunnel up and down with the service. Remove them to keep the instance on your local network only. See [exposing it safely](/docs/exposing).
...
Point a client at it from [getting started](/docs), understand what actually happens during a run in [how OpenBrowse works](/docs/concepts), or read the [v3 API reference](/docs/api).
