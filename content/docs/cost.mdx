...
A budget stop resolves its output the same way an ordinary finish does, rather than discarding the work and charging you for nothing. The answer store's contents stand when the store holds them, `result.json` otherwise, and `isTaskSuccessful` is true when that output matches the requested schema. Either way the output is written, so null or partial values remain visible and the caller can decide whether the delivered result is sufficient before escalating.

The salvage never calls the LLM. The budget that ended the run is the same budget schema repair would spend.

One consequence worth knowing: a capped run whose store is schema-valid can carry `isTaskSuccessful: true` **and** `failureKind: "budget_exceeded"`. The first says a result was delivered; the second says the run stopped at its spending limit, so read them together.
...
5. **Fields you already know.** Anything recoverable from one plain HTTP GET — the title, the meta description, `og:` tags, the page's own links — costs nothing to fetch yourself and a browser step each to have an agent read back. Hand them over as schema `default`s and the agent never spends a token on them. On one org-enrichment task this was the difference between 30 steps and 8.
6. **Fields that only apply sometimes.** A field that is null by design on the branch the run took is still chased by the completeness gate unless the schema says otherwise. Both keywords are described under [structured output](/docs/structured-output#telling-the-schema-what-you-already-know).
