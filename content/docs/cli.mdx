---
title: "The openbrowse command"
description: "Every openbrowse subcommand: running the server, managing the systemd service, checking for and installing updates, tuning the host, and removing it again."
---
...
The service is covered in full under [running it as a service](/docs/installation#run-it-as-a-service), including what happens when you already have a unit of your own.
...
`check-update` reports the installed version against the latest on PyPI and exits non-zero only if the check itself failed, so "already up to date" is a success. `update` runs the upgrade through whichever manager owns this copy, `uv tool upgrade`, `pipx upgrade`, pip inside its virtual environment or a `git pull`, and then tells you to restart; unlike the dashboard's button, it does not restart the server for you. Where the owning manager cannot be reached it names the install method and declines rather than running something that would fail. [Updating](/docs/installation#updating) has the full table.

The dashboard does the same two things without a shell: a badge in the navigation when a release is waiting, and a one-click **Install and restart** on the Settings page that is refused while any session is running. [Updating](/docs/installation#updating) covers both paths and the one thing to do afterwards.
...
What it actually writes, and why a Raspberry Pi needs it, is under [sizing it for your machine](/docs/installation#sizing-it-for-your-machine). Run it again after every upgrade: the sudoers grant it writes names the script by its full path, and that path moves with the package.
...
<Callout type="warn">
Your data directory holds the profile cookie jars, which are live credentials for every site those profiles are logged into, and they are not recoverable once removed. Pass `--keep-data` if you intend to reinstall.
</Callout>
