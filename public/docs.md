...
Open `http://<your-host>:8420`. A fresh install takes you straight to a one-time setup wizard at `/setup`, which walks you through a dashboard password, a model provider key, how much of the machine OpenBrowse may use, and the API bearer key it generates for you, then writes `.env`. Restart the server after it finishes so the new configuration is picked up; once any credential exists the setup routes disappear and the dashboard asks for the password you chose.
