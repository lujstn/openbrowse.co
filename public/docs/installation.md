...
| Host tuning | Offers the `openbrowse tune` command for this machine. Optional, and you tell the wizard whether you ran it |
| Browser download | Fetches the stealth Chromium build now, with progress, rather than leaving it to your first session |
| Restart | Applies the configuration and hands you to the dashboard |
...
| `DEFAULT_MODEL` | Optional. The model a request that names none runs on. Left unset, the default follows whichever provider key is configured, `gpt-5.6-terra` for OpenAI and `claude-sonnet-5` for Anthropic. The v3 API and the dashboard's run form both follow it |
