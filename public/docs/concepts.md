...
*Source: https://openbrowse.co/docs/concepts*
...
Before the map, a word on when this is the wrong tool. Everything below describes an agent: you state an outcome, and it works the site out for itself, deciding what to click and what to read until the answer is complete. That machinery is worth paying for when you do not already know which pages hold the answer. When you do know, a crawler is faster, cheaper and more predictable, and Firecrawl is the obvious one to reach for: hand it URLs, get clean page content back at scale. The two overlap enough that people ask, so here is the dividing line as Firecrawl's own documentation draws it. Their open-source stack ships the crawl, scrape, map and search routes, while Agent, Browser, `interact` and `feedback` are among the capabilities their docs name Firecrawl Cloud for. Self-host theirs and you have the crawler; the agent stays in their cloud. Self-host this and the agent is the thing you are running.
...
This is also why the first line of your task matters more than the rest; [writing tasks](https://openbrowse.co/docs/tasks) covers how to use that.
...
Everything is one Python process plus per-session subprocesses (Xvfb, x11vnc, websockify, Chromium). State lives in SQLite at `data/browser_use.db`, profiles under `data/profiles/`, both relative to [OpenBrowse's home directory](https://openbrowse.co/docs/installation#where-openbrowse-keeps-its-files), and the whole thing listens on port 8420. There is no queue service, no external database, and no cloud dependency beyond the LLM providers themselves.
