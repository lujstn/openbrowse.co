...
Before the map, a word on when this is the wrong tool. Everything below describes an agent: you state an outcome, and it works the site out for itself, deciding what to click and what to read until the answer is complete. That machinery is worth paying for when you do not already know which pages hold the answer. When you do know, a crawler is faster, cheaper and more predictable, and Firecrawl is the obvious one to reach for: hand it URLs, get clean page content back at scale. The two overlap enough that people ask, so here is the dividing line as Firecrawl's own documentation draws it. Their open-source stack ships the crawl and scrape routes, while Agent, Browser, `interact` and `feedback` are listed as capabilities to use Firecrawl Cloud for. Self-host theirs and you have the crawler; the agent stays in their cloud. Self-host this and the agent is the thing you are running.
